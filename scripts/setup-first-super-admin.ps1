...
$backendPath = Join-Path $PSScriptRoot "..\Agenchq"
if (-not (Test-Path $backendPath)) {
    Write-Host "Error: Agenchq directory not found" -ForegroundColor Red
    exit 1
}
...
Set-Location ..
$envContent = Get-Content "Agenchq\.env" -Raw -ErrorAction SilentlyContinue
...
if ($envContent -and $envContent -match "SUPER_ADMIN_SECRET_KEY\s*=\s*(.+)") {
    ...
    if ($envContent -notmatch "\n$") {
        Add-Content -Path "Agenchq\.env" -Value "`n"
    }
    Add-Content -Path "Agenchq\.env" -Value "`n# Super Admin Secret Key`nSUPER_ADMIN_SECRET_KEY=$secretKey"
