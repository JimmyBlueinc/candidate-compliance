# Display the current SUPER_ADMIN_SECRET_KEY from Agenchq/.env

$backendEnvPath = Join-Path $PSScriptRoot "..\Agenchq\.env"

if (-not (Test-Path $backendEnvPath)) {
    Write-Host "Error: Agenchq/.env not found." -ForegroundColor Red
    exit 1
}
...
if ($envContent -match "SUPER_ADMIN_SECRET_KEY\s*=\s*(.+)") {
    ...
    Write-Host "`n✓ SUPER_ADMIN_SECRET_KEY found in Agenchq/.env" -ForegroundColor Green
    ...
    Write-Host "SUPER_ADMIN_SECRET_KEY not found in Agenchq/.env" -ForegroundColor Red
