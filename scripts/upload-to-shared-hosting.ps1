...
$backendDir = Join-Path $uploadDir "Agenchq"
...
Get-ChildItem -Path "Agenchq" -Recurse | ForEach-Object {
    $relativePath = $_.FullName.Replace((Resolve-Path "Agenchq").Path, "").TrimStart("\")
...
if (Test-Path (Join-Path "Agenchq" "vendor")) {
    ...
    Write-Host "  cd Agenchq; composer install --optimize-autoloader --no-dev" -ForegroundColor Gray
}
Write-Host ""
...
3. Create an Agenchq folder (outside public_html if possible)
4. Upload all files from Agenchq folder to your hosting backend folder
5. Structure should be:
   /home/username/Agenchq/
   - app/
   - public/
   - storage/
   - ... (all other files)

6. Create .env file in Agenchq folder:
...
8. Make backend accessible:
   Option A: Create subdomain api.yourdomain.com pointing to Agenchq/public/
   Option B: Copy Agenchq/public/ contents to public_html/api/
...
2. Add new cron:
   ...
   - Command: /usr/bin/php /home/username/Agenchq/artisan schedule:run
...
Write-Host "  - Agenchq/          -> Upload to your hosting backend folder" -ForegroundColor White
...
Write-Host "2. Upload Agenchq/ folder via File Manager or FTP" -ForegroundColor White
