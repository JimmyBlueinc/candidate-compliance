...
REM Update Agenchq .env
echo Updating Agenchq configuration...
if exist Agenchq\.env (
    findstr /v "FRONTEND_URL" Agenchq\.env > Agenchq\.env.tmp 2>nul
    move /y Agenchq\.env.tmp Agenchq\.env >nul
)
echo FRONTEND_URL=http://%IP%:5173 >> Agenchq\.env
...
start "Laravel Backend" cmd /k "cd Agenchq && php artisan serve --host=0.0.0.0 --port=8000"
