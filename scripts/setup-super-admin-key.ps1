# Script to automatically add SUPER_ADMIN_SECRET_KEY to Agenchq/.env

$backendEnvPath = Join-Path $PSScriptRoot "..\Agenchq\.env"

if (-not (Test-Path $backendEnvPath)) {
    Write-Host "Error: Agenchq/.env not found. Please ensure it exists." -ForegroundColor Red
    exit 1
}
...
if ($envContent -match "SUPER_ADMIN_SECRET_KEY\s*=") {
    Write-Host "SUPER_ADMIN_SECRET_KEY already exists in Agenchq/.env" -ForegroundColor Yellow
...
Write-Host "✓ SUPER_ADMIN_SECRET_KEY added to Agenchq/.env" -ForegroundColor Green
