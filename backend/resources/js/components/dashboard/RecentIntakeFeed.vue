<template>
  <div class="glass-dark rounded-[32px] p-8 flex flex-col border border-white/5">
    <div class="flex items-center justify-between mb-6">
      <div>
        <h3 class="font-display text-xl text-white">Recent Web Intake</h3>
        <p class="text-xs text-[color:var(--p-text-muted-color)] font-medium">Last 5 candidates received</p>
      </div>
      <button
        type="button"
    ...
    <div v-if="loading" class="text-sm text-[color:var(--p-text-muted-color)]">
      Loading...
    </div>

    <div v-else-if="items.length === 0" class="text-sm text-[color:var(--p-text-muted-color)]">
...
function normalizedTags(tags) {
    ...
    return list
        .map((t) => String(t || '').trim())
        .filter((t) => t && t.toLowerCase() !== 'web-intake');
}
