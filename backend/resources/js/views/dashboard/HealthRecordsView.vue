    ...
    <Card>
      <template #content>
        <div class="flex items-center justify-between">
          <div>
            <h2 class="font-display text-2xl">Health Records</h2>
            <p class="text-sm text-[color:var(--p-text-muted-color)]">Review and manage health record documents.</p>
          </div>
...
import HealthRecordFormModal from '../../components/dashboard/HealthRecordFormModal.vue';
...
async function fetchRecords() {
    try {
        loading.value = true;
        error.value = '';
        const path = `/health-records${queryString.value ? `?${queryString.value}` : ''}`;
        const res = await apiGet(path);
        rows.value = normalizeApiList(res);
    } catch (e) {
        rows.value = [];
        error.value = e?.response?.data?.message || e?.message || 'Failed to load health records';
    } finally {
        loading.value = false;
    }
}
...
async function handleDelete(id) {
    ...
    try {
        await apiDelete(`/health-records/${encodeURIComponent(String(id))}`);
        await fetchRecords();
    } catch (e) {
