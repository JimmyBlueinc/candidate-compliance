...
import WorkAuthorizationFormModal from '../components/WorkAuthorizationFormModal';
...
const WorkAuthorizations: React.FC = () => {
  ...
  const fetchRecords = useCallback(async () => {
    try {
      setLoading(true);
      setError('');
      const res: any = await request(`/work-authorizations${query ? `?${query}` : ''}`);
      const data = normalize(res);
      setRows(prev => JSON.stringify(prev) === JSON.stringify(data) ? prev : data);
    } catch (e: any) {
      setRows([]);
      setError(e?.message || 'Failed to load work authorizations');
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (id: string | number) => {
    ...
    try {
      await request(`/work-authorizations/${id}`, { method: 'DELETE' });
      fetchRecords();
    } catch (e: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="glass-dark rounded-3xl border border-white/5 p-6 flex items-center justify-between">
        <div>
          <h2 className="font-display text-2xl text-white">Work Authorizations</h2>
          <p className="text-slate-500 text-sm">Review and manage work authorization records.</p>
        </div>
        <button
          onClick={handleAdd}
            ...
            {loading ? (
              ...
              rows.map((r) => (
                <tr key={r.id} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-6 py-4 text-slate-400">{r.authorization_type || '—'}</td>
                  <td className="px-6 py-4 text-slate-200">{r.status || '—'}</td>
