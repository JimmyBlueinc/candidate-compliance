...
const HealthRecordFormModal: React.FC<HealthRecordFormModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  initialData,
  ...
  const [formData, setFormData] = useState({
    ...
    record_type: initialData?.record_type || 'immunization',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const endpoint = isEdit ? `/health-records/${initialData.id}` : '/health-records';
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold uppercase tracking-widest text-slate-500">Record Type</label>
              <select
            ...
            <div className="relative">
              ...
              >
                ...
                <span className="text-sm text-slate-400 group-hover:text-slate-200">
                  {file ? file.name : (initialData?.document_path ? 'Replace existing document' : 'Upload health record document')}
                </span>
              </label>
            </div>
          </div>
