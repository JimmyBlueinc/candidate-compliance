...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (DB::getDriverName() === 'sqlite') {
            ...
            DB::statement("ALTER TABLE users ADD CONSTRAINT users_role_check CHECK (role IN ('platform_admin', 'org_super_admin', 'admin', 'candidate'))");
        } else {
            // For other databases (MySQL), we can alter the column
    ...
    public function down(): void
    {
        if (DB::getDriverName() === 'sqlite') {
            ...
            DB::statement("ALTER TABLE users ALTER COLUMN role SET NOT NULL");
            DB::statement("ALTER TABLE users DROP CONSTRAINT IF EXISTS users_role_check");
            DB::statement("ALTER TABLE users ADD CONSTRAINT users_role_check CHECK (role IN ('platform_admin', 'org_super_admin', 'admin'))");
        } else {
            Schema::table('users', function (Blueprint $table) {
                $table->enum('role', ['platform_admin', 'org_super_admin', 'admin'])
                    ->default('admin')
                    ->change();
            });
        }
