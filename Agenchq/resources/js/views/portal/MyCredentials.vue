        ...
        >
          <div class="flex items-start justify-between gap-4">
            <div class="min-w-0">
              ...
              <div v-if="(row.status === 'rejected' || row.status === 'needs_correction') && row.latest_rejection_reason" class="mt-2 text-xs text-red-400">
                {{ row.status === 'needs_correction' ? 'Needs correction:' : 'Rejected:' }} {{ row.latest_rejection_reason }}
              </div>
            </div>
        ...
        >
          <div class="flex items-start justify-between gap-4">
            <div class="min-w-0">
              ...
              <div v-if="r.rejection_reason" class="mt-2 text-xs text-red-300">
                {{ r.status === 'needs_correction' ? 'Needs correction:' : 'Rejected:' }} {{ r.rejection_reason }}
              </div>
...
function requirementStatusStyle(r) {
    ...
    if (s === 'needs_correction') return { borderColor: 'rgba(245,158,11,0.4)', backgroundColor: 'rgba(245,158,11,0.14)', color: 'rgba(251,191,36,1)' };
...
function statusStyle(row) {
    ...
    if (s === 'needs_correction') {
        return { backgroundColor: 'rgba(245, 158, 11, 0.14)', borderColor: 'rgba(245, 158, 11, 0.3)', color: 'rgb(245, 158, 11)' };
    }
    return { borderColor: 'rgba(148,163,184,0.25)', backgroundColor: 'rgba(148,163,184,0.08)', color: 'rgba(148,163,184,1)' };
}
