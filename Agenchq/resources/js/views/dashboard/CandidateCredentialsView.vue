        ...
        <DataTable :value="rows" :loading="loading" paginator :rows="12" responsiveLayout="scroll" dataKey="id">
          ...
          <Column field="status" header="Status" style="width: 160px">
            <template #body="{ data }">
              ...
              <div v-if="(data.status === 'rejected' || data.status === 'needs_correction') && data.latest_rejection_reason" class="text-xs text-red-300 mt-1">
...
function statusSeverity(v) {
    ...
    if (v === 'needs_correction') return 'warning';
...
async function refresh() {
    ...
    try {
        ...
        rows.value = normalizeApiList(payload?.credentials);
        candidateName.value = payload?.candidate?.name || '';
    } catch (e) {
        rows.value = [];
        candidateName.value = '';
