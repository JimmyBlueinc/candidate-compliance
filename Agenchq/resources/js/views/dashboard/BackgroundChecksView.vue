    ...
    <Card>
      <template #content>
        <div class="flex items-center justify-between">
          <div>
            <h2 class="font-display text-2xl">Background Checks</h2>
            <p class="text-sm text-[color:var(--p-text-muted-color)]">Review and manage background check records.</p>
          </div>
    ...
    <Card>
      <template #content>
        <DataTable :value="rows" :loading="loading" dataKey="id" stripedRows responsiveLayout="scroll">
          ...
          <Column field="check_type" header="Type">
            <template #body="{ data }">
              <span>{{ data.check_type || '—' }}</span>
            </template>
          </Column>
...
import BackgroundCheckFormModal from '../../components/dashboard/BackgroundCheckFormModal.vue';
...
async function fetchRecords() {
    try {
        loading.value = true;
        error.value = '';
        const path = `/background-checks${queryString.value ? `?${queryString.value}` : ''}`;
        const res = await apiGet(path);
        rows.value = normalizeApiList(res);
    } catch (e) {
        rows.value = [];
        error.value = e?.response?.data?.message || e?.message || 'Failed to load background checks';
    } finally {
        loading.value = false;
    }
}
...
async function handleDelete(id) {
    ...
    try {
        await apiDelete(`/background-checks/${encodeURIComponent(String(id))}`);
        await fetchRecords();
    } catch (e) {
