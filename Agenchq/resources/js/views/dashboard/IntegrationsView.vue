...
const INTEGRATION_LOGO_MAP = {
  ...
  quickbooks: [
    'https://cdn.jsdelivr.net/npm/simple-icons@latest/icons/quickbooks.svg',
    'https://cdn.simpleicons.org/quickbooks',
  ],
  zapier: [
    'https://cdn.simpleicons.org/zapier',
    'https://cdn.jsdelivr.net/npm/simple-icons@latest/icons/zapier.svg',
  ],
};
...
function integrationLogo(key) {
  const fallbackList = INTEGRATION_LOGO_MAP[String(key || '').toLowerCase()];
...
function onLogoError(event) {
  ...
  const integrationKey = Object.keys(INTEGRATION_LOGO_MAP).find((k) => key.includes(k.replace('_', ' ')));
  const sources = integrationKey ? INTEGRATION_LOGO_MAP[integrationKey] : [];
  const index = Number(img.dataset.logoIndex || '0');
