      ...
      <template #actions>
        <div class="relative">
          ...
          >
            ...
            <div class="mt-2 pt-2 border-t border-[color:var(--aq-border)] text-[10px] text-[color:var(--aq-muted)]">
              Drag cards to reorder. Use +/- on each card to resize.
            </div>
          </div>
        </div>
      ...
      >
        <div class="dashboard-widget-toolbar">
          <div class="dashboard-widget-handle" :title="`Drag to reorder: ${widgetLabel(widgetKey)}`">
            ...
            <span class="dashboard-widget-size-pill">{{ widgetSize(widgetKey).toUpperCase() }}</span>
          </div>
          <div class="dashboard-widget-size-controls">
            <button type="button" class="dashboard-widget-size-btn" title="Reduce size" @click.stop="resizeWidget(widgetKey, -1)">-</button>
            <button type="button" class="dashboard-widget-size-btn" title="Increase size" @click.stop="resizeWidget(widgetKey, 1)">+</button>
          </div>
        </div>
...
const role = computed(() => String(auth.user?.role || ''));
const isFinanceRole = computed(() => role.value === 'finance');
const isSuperAdmin = computed(() => role.value === 'org_super_admin');
...
const widgetMeta = {
  facilityProfitability: { label: 'Facility Profitability', defaultSize: 'lg' },
  complianceTrend: { label: 'Compliance Trend', defaultSize: 'md' },
  riskExposure: { label: 'Risk Exposure', defaultSize: 'md' },
  activityFeed: { label: 'Activity Feed', defaultSize: 'md' },
  notifications: { label: 'Notifications', defaultSize: 'md' },
};
const widgetSizeOrder = ['sm', 'md', 'lg', 'xl'];
const widgetSizeLayoutClass = {
  sm: 'col-span-12 md:col-span-6 lg:col-span-4',
  md: 'col-span-12 lg:col-span-5',
  lg: 'col-span-12 lg:col-span-7',
  xl: 'col-span-12',
};
...
function widgetLayoutClass(key) {
  const size = widgetSize(key);
  return widgetSizeLayoutClass[size] || widgetSizeLayoutClass.md;
}

function widgetSize(key) {
  const saved = String(ui.dashboardWidgetSizes?.[key] || '');
  if (widgetSizeOrder.includes(saved)) return saved;
  const fallback = String(widgetMeta[key]?.defaultSize || 'md');
  return widgetSizeOrder.includes(fallback) ? fallback : 'md';
}

function resizeWidget(key, direction) {
  const current = widgetSize(key);
  const currentIndex = widgetSizeOrder.indexOf(current);
  const nextIndex = Math.min(widgetSizeOrder.length - 1, Math.max(0, currentIndex + Number(direction || 0)));
  const next = widgetSizeOrder[nextIndex];
  if (!next || next === current) return;
  ui.setDashboardWidgetSize(key, next);
}

function widgetAllowedByFlags(key) {
  // Sensitive cross-domain analytics are reserved for super admins.
  if (['complianceTrend', 'riskExposure', 'activityFeed'].includes(key) && !isSuperAdmin.value) {
    return false;
  }

  // Finance users should not see org-wide notification feed from this page.
  if (key === 'notifications' && isFinanceRole.value && !isSuperAdmin.value) {
    return false;
  }
...
.dashboard-widget-size-btn {
  border: 1px solid color-mix(in srgb, var(--aq-border) 85%, transparent);
  border-radius: var(--radius-md);
  background: color-mix(in srgb, var(--aq-surface-2) 92%, transparent);
  color: var(--aq-fg);
  width: 1.45rem;
  height: 1.45rem;
  font-size: 0.85rem;
...
.dashboard-widget-size-btn:hover {
  border-color: color-mix(in srgb, var(--aq-primary) 40%, transparent);
  color: var(--aq-primary);
}

.dashboard-widget-size-pill {
  margin-left: 0.25rem;
