...
const DASHBOARD_WIDGET_SIZES_KEY = 'ui.dashboardWidgetSizes';
const DEFAULT_DASHBOARD_WIDGET_ORDER = ['facilityProfitability', 'complianceTrend', 'riskExposure', 'activityFeed', 'notifications'];
const DEFAULT_DASHBOARD_WIDGET_SIZES = {
    facilityProfitability: 'lg',
    complianceTrend: 'md',
    riskExposure: 'md',
    activityFeed: 'md',
    notifications: 'md',
...
export const useUiStore = defineStore('ui', {
    state: () => ({
        ...
        dashboardWidgetSizes: { ...DEFAULT_DASHBOARD_WIDGET_SIZES },
    }),
    ...
    actions: {
        initTheme() {
            ...
            try {
                const widgetSizesRaw = localStorage.getItem(DASHBOARD_WIDGET_SIZES_KEY);
                if (widgetSizesRaw) {
                    const parsed = JSON.parse(widgetSizesRaw);
                    if (parsed && typeof parsed === 'object') {
                        this.dashboardWidgetSizes = {
                            ...this.dashboardWidgetSizes,
                            ...parsed,
                        };
                    }
                ...
                this.dashboardWidgetSizes = { ...DEFAULT_DASHBOARD_WIDGET_SIZES };
            }
        },
        ...
        async syncFromServer() {
            ...
            try {
                ...
                if (settings?.dashboard_widget_sizes && typeof settings.dashboard_widget_sizes === 'object') {
                    this.dashboardWidgetSizes = {
                        ...this.dashboardWidgetSizes,
                        ...settings.dashboard_widget_sizes,
                    };
                    localStorage.setItem(DASHBOARD_WIDGET_SIZES_KEY, JSON.stringify(this.dashboardWidgetSizes));
                }
        ...
        setDashboardWidgetSize(widgetKey, size, options = {}) {
            const persist = options.persist !== false;
            if (!Object.prototype.hasOwnProperty.call(this.dashboardWidgets, widgetKey)) return;
            ...
            if (persist) {
                this.persistToServer({ dashboard_widget_sizes: this.dashboardWidgetSizes });
            }
        },
    },
});
