...
import HealthRecordFormModal from '../components/HealthRecordFormModal';
...
const HealthRecords: React.FC = () => {
  ...
  const fetchRecords = useCallback(async () => {
    try {
      setLoading(true);
      setError('');
      const res: any = await request(`/health-records${query ? `?${query}` : ''}`);
  ...
  return (
    <div className="space-y-6">
      <div className="glass-dark rounded-3xl border border-white/5 p-6 flex items-center justify-between">
        <div>
          <h2 className="font-display text-2xl text-white">Health Records</h2>
          <p className="text-slate-500 text-sm">Review and manage health record documents.</p>
        </div>
        <button
          onClick={handleAdd}
            ...
            {loading ? (
              ...
              rows.map((r) => (
                <tr key={r.id} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-6 py-4 text-slate-400">{r.record_type || '—'}</td>
