...
interface BackgroundCheckFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  initialData?: any;
...
const BackgroundCheckFormModal: React.FC<BackgroundCheckFormModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  initialData,
  ...
  const [formData, setFormData] = useState({
    ...
    check_type: initialData?.check_type || 'criminal_record_check',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const endpoint = isEdit ? `/background-checks/${initialData.id}` : '/background-checks';
      ...
      setError(err.message || 'Failed to save background check');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="glass-dark w-full max-w-2xl rounded-3xl border border-white/10 shadow-2xl overflow-hidden animate-[fadeInUp_0.3s_ease-out]">
        <div className="flex items-center justify-between p-6 border-b border-white/5">
          <h3 className="font-display text-xl text-white">
            {initialData ? 'Edit Background Check' : 'Add Background Check'}
          </h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold uppercase tracking-widest text-slate-500">Check Type</label>
              <select
            ...
            <div className="relative">
              ...
              >
                ...
                <span className="text-sm text-slate-400 group-hover:text-slate-200">
                  {file ? file.name : (initialData?.document_path ? 'Replace existing document' : 'Upload background check document')}
                </span>
              </label>
            </div>
          </div>
...
export default BackgroundCheckFormModal;
